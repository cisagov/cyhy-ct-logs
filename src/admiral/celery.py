...
def determine_input(description, cli_arg, env_key, default):
    '''determine where a setting is coming from and tell the user.
    ...
    if env_key in os.environ.keys():
        ...
        print(f'{description} specified in environment variable {env_key}: {env_value}')
        return env_value

    print(f'{description} not specified on commandline or environment.  Using default: {default}')
    return default
